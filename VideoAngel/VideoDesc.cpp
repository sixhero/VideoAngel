...
int VideoDesc::GetVideoData(VideoData* video_data)
{
	...
	int ret;
	int video_size = av_image_get_buffer_size(AV_PIX_FMT_RGB32, m_video_width, m_video_height, 4);
	if (video_size <= 0)
	{
	...
	//视频格式数据转换，转码为RGB32
	ret = sws_scale(m_video_sws_context, frame->data, frame->linesize, 0, m_video_height, m_av_frame_dest->data, m_av_frame_dest->linesize);
	if (ret < 0)
	{
...
int VideoDesc::GetAudioData(AudioData* audio_data)
{
	...
	audio_data->pts = 1000 * frame->pts * av_q2d(m_av_format_context->streams[m_audio_index]->time_base);
	 ...
	 //绑定填充要转换的目的帧
	av_samples_fill_arrays(m_av_frame_dest->data, m_av_frame_dest->linesize, audio_data->_data, frame->ch_layout.nb_channels, frame->nb_samples, AV_SAMPLE_FMT_S16, 1);

	//音频编码转换将数据取出
	ret = swr_convert(m_audio_swr_context, m_av_frame_dest->data, frame->nb_samples, (const uint8_t**)frame->data, frame->nb_samples);
	if (ret < 0)
	{
...
{
	if (m_video_frame_queue.size() >= 10)
	{
		return -1;
	}
	//开始读取数据源的数据包
